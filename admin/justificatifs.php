        ...
        switch ($_POST['action']) {
            ...
            case 'changerStatut':
                try {
                    ...
                    // Si le justificatif est accepté, mettre à jour les présences
                    if ($_POST['statut'] === 'accepte') {
                        // Récupérer les informations du justificatif
                        $stmt = $conn->prepare("
                            SELECT etudiant_id, date_debut, date_fin
                            FROM justificatifs
                            WHERE id = ?
                        ");
                        $stmt->execute([$_POST['justificatif_id']]);
                        ...
                        if ($justificatif) {
                            // Mettre à jour toutes les présences dans la période du justificatif
                            $stmt = $conn->prepare("
                                UPDATE presences
                                SET statut = 'present'
                                WHERE etudiant_id = ?
                            ...
                            $stmt->execute([
                                ...
                                $justificatif['date_debut'],
                                $justificatif['date_fin']
                            ]);
                        }
                    }
